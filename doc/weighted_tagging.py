...
def process_file(filename: str, print_to_console: bool = False) -> Dict[str, int]:
    """
    ...
    keylist = construct_keylist(
        keylist_path="input/keylist_total.csv", alternate_lists="all")
    ...
    word_counts_end = filter_values(word_counts_weighed, 2)
...
def scan_folder(input_path: str, output_path: str = "output/csv", print_to_console: bool = False) -> None:
    """
    ...
    output_path (str): Path of the folder in which the csv should be stored
    ...
    for file_name in listdir(input_path):
        file_path = ospath.join(input_path, file_name)
        if ospath.isfile(file_path):
            nested_dict[file_name] = process_file(file_path)
    dataframe = nested_dict_to_dataframe(dict)
    save_dataframe(dataframe, output_path)
    if print_to_console == True:
        print(dataframe)
    return None

text = "Taberlet, P ., Coissac, E., Pompanon, F., Brochmann, C. & Willerslev, E. Towards next-generation biodiversity assessment using DNA metabarcoding. Molecular Ecology 21, 2045-2050 (2012).2. Shokralla, S. et al. Massively parallel multiplex DNA sequencing for specimen identification using an IlluminaMiSeq platform. Scientific reports  5, 9687 (2015).3. Ji, Y . Q. et al. Reliable, verifiable and efficient monitoring of biodiversity via metabarcoding. Ecology Letters 16, 1245-1257 (2013).4. Shokralla, S. et al.  Next-generation DNA barcoding: using next-generation sequencing to enhance and accelerate DNA barcode capture from single specimens. Molecular Ecology Resources 14, 892-901 (2014).5. "

if __name__ == "__main__":
    process_file("input/pdf/docs/Alberdi et al. - 2018 - Scrutinizing key steps for reliable metabarcoding .txt", print_to_console=True)
    # scan_folder("input/pdf/docs/corrected", True)