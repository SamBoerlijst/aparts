...
def tag_file_weighted(filename: str, keylist_path: str = "input/keylist.csv", alternate_lists: str ="all", treshold: int = 2, print_to_console: bool = False) -> dict[str, int]:
    ...
    """
    section_dictionary = wt.split_text_to_sections(filename)
    keylist = construct_keylist(
        keylist_path, alternate_lists)
    word_counts = wt.count_keyword_occurrences(section_dictionary, keylist)
    word_counts_filtered = wt.filter_values(word_counts, 1)
    word_counts_weighed = wt.weigh_keywords(word_counts_filtered)
    word_counts_end = wt.filter_values(word_counts_weighed, treshold)
    denested = wt.denest_and_order_dict(word_counts_end)
    if print_to_console == True:
        wt.print_nested_dict(denested)
    return denested
